...
class TestFindFuture < Minitest::Test
  ...
  def test_only_positives
    assert_equal 1, RubyCodingChallenges::MinimumProduct.search(@test_only_positives)
    assert_equal 2, RubyCodingChallenges::MinimumProduct.search(@test_only_positives_mix)
  end

  # Se não há negativos e há pelo menos um zero, a função deve devolver 0
  def test_contains_zeros_without_negatives
    assert_equal 0, RubyCodingChallenges::MinimumProduct.search(@test_contains_zeros_without_negatives)
  end

  # Com um único negativo, o produto total fica negativo e é o mínimo
  def test_one_negative
    assert_equal (-9), RubyCodingChallenges::MinimumProduct.search(@test_one_negative)
  end

  # Se há um número ímpar de negativos, o produto total será negativo e já é mínimo
  def test_negatives_odd
    assert_equal (-120), RubyCodingChallenges::MinimumProduct.search(@test_negatives_odd)
  end
  ...
  def test_negatives_pair
    assert_equal (-24), RubyCodingChallenges::MinimumProduct.search(@test_negatives_pair)
  end
  ...
  def test_only_negatives
    ...
    assert_equal (-2), RubyCodingChallenges::MinimumProduct.search(@test_only_negatives_pair)          
    # Três negativos (ímpar) → produto total (ímpar de negativos => negativo)
    assert_equal (-6), RubyCodingChallenges::MinimumProduct.search(@test_only_negatives_odd)
  end

  # Mesmo com zeros, dependendo do mix o resultado pode ser 0 (por total_product = 0)
  def test_array_with_zero_and_negatives 
    assert_equal (0), RubyCodingChallenges::MinimumProduct.search(@test_array_with_zero_and_negatives)
  end
end